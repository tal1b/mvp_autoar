...
def add_employee():
    """Добавление сотрудника (только для менеджера)"""
    if not is_manager():
        flash('Access denied', 'error')
    ...
    if request.method == 'POST':
        ...
        required = ['full_name', 'position', 'department', 'start_date', 'username', 'password']
        missing = validate_required_fields(data, required)
        
        if missing:
            flash(f'Required fields are missing: {", ".join(missing)}', 'error')
            return render_template('employee_form.html', employee=None, is_manager=True)
        ...
        # Создаем запись сотрудника
        cursor = conn.cursor()
        cursor.execute('''INSERT INTO employees (full_name, position, department, start_date, email, phone)
                       VALUES (?, ?, ?, ?, ?, ?)''',
        ...
        cursor.execute('''INSERT INTO users (name, username, password, role, employee_id)
                       VALUES (?, ?, ?, ?, ?)''',
                    (data['full_name'], data['username'], password_hash, data['role'], employee_id))
...
def edit_employee(employee_id):
    ...
    if request.method == 'POST':
        ...
        if missing:
            ...
            conn.close()
            return render_template('employee_form.html', employee=employee, current_role=user['role'] if user else None, is_manager=True)
